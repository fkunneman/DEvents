    ...
    def add_event(self,event):
        ...
        for term in event.entities:
            ...
            if len(sequence["dates"]) > 1:
                        ...
                        if len(sequence["last_periodic"]) > 0:
                            ...
                            if last_periodic[3] == len(sequence["merged_intervals"])-1: #periodicity until last date
                                ...
                                if stdev < 10: #update current sequence
                                    term_stdev[index] = [stdev,", ".join(sequence["merged_dates"][last_periodic[2]:last_periodic[3]+1]),",".join(intervals + [merged_interval])]
                                ...
                                if stdev < 10:
                                    ...
                                    sequence["merged_intervals"] = intervals + [sum(sequence["merged_intervals"][last_periodic[3]:])]
                                ...
                                if len(intervals) >= 2: #find best periodicity
                                    ...
                                    if best[1] < 10:
                                        ...
                                        term_stdev[index+1] = [best[1],", ".join(sequence["merged_dates"][last_periodic[3]+best[0]:]),",".join(sequence["merged_intervals"][last_periodic[3]+best[0]:])]
                            ...
                            if len(intervals) >= 2: #find best periodicity
                                ...
                                if best[1] < 10:
                                    ...
                                    term_stdev[0] = [best[1],", ".join(sequence["merged_dates"][best[0]:]),",".join(sequence["merged_intervals"][best[0]:])]
                                                            # try:      
                ...
                #     self.string_events[self.strings].append(event)
                #     self.strings += 1
            print(term,sequence["merged_intervals"],term_stdev[term])
